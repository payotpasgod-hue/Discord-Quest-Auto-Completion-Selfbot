...
export class ClientQuest extends Client {
  ...
  public notifier: WebhookNotifier;
  ...
  constructor(token: string, webhookUrl?: string) {
    ...
    this.notifier = new WebhookNotifier(webhookUrl ?? process.env.WEBHOOK_URL);
  ...
  connect() {
    return Promise.allSettled([
      ...
      .catch((error: Error) => {
        ...
        return this.sendWebhookMessage('Error during client connection: ' + error.message);
      });
  }
  ...
  async destroy(): Promise<void> {
    try {
      ...
      console.warn('Failed to destroy websocket client cleanly:', error);
    }
  }

  async notifyStartup(username?: string): Promise<void> {
    await this.notifier.startup(username);
  }

  async notifySummary(total: number, processed: number, failed: number): Promise<void> {
    await this.notifier.summary(total, processed, failed);
  }

  async notifyError(message: string): Promise<void> {
    await this.notifier.error(message);
  }

  async notifyQuestCompleted(name: string, questId: string): Promise<void> {
    await this.notifier.completed(name, questId);
  }
  ...
  emitQuestCompleted(questId: string, questName?: string) {
    if (questName) {
      return this.notifyQuestCompleted(questName, questId);
    }
